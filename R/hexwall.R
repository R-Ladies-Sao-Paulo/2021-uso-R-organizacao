...
hexwall <-
  function(path,
    ...
    bad_size <- stickers %>%
      map_lgl( ~ remove_size &&
                 with(image_info(.x), height < (median(height) - 2) |
    ...
    if (is.null(coords)) {
      ...
      sticker_col_size <-
        ceiling(length(stickers) / (sticker_row_size - 0.5))
      row_lens <-
        rep(c(sticker_row_size, sticker_row_size - 1), length.out = sticker_col_size)
      row_lens[length(row_lens)] <-
        row_lens[length(row_lens)]  - (length(stickers) - sum(row_lens))
      sticker_rows <- map2(row_lens,
                           cumsum(row_lens),
      ...
      canvas <- image_blank(
        ...
        sticker_height + (sticker_col_size - 1) * sticker_height /
          1.33526,
        "white"
      )
      reduce2(
